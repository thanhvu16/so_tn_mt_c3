...
use Modules\VanBanDen\Entities\VanBanDen2021;
...
//        $dsLoaiVanBan = VanBanDenOld::query()->first();
    ...
    public function index(Request $request)
    {
        ...
        $user = auth::user();
        $donViVanThu = DonVi::where('id', $donVi->id)->first();
...
//                ->get();
        ...
        $ds_loaiVanBan = LoaiVanBan::wherenull('deleted_at')->whereIn('loai_van_ban', [1, 3])->orderBy('ten_loai_van_ban', 'asc')->get();
    ...
    public function create()
    {
        ...
        $loaivanban = LoaiVanBan::wherenull('deleted_at')->whereIn('loai_van_ban', [1, 3])->orderBy('ten_loai_van_ban', 'asc')->get();
...
//                'so_van_ban_id' => $soVanBan->id,
    ...
    public function laysoden(Request $request)
    {
        ...
        if (auth::user()->hasRole(VAN_THU_HUYEN)) {
            ...
            $donViVanThu = DonVi::where('id', auth::user()->don_vi_id)->first();
            if ($donViVanThu->cap_chi_nhanh == 1) {
                $soDenvb = VanBanDen::where([
                    'van_ban_chi_nhanh' => $donViVanThu->id,
                    'type' => 2
                ])->whereYear('ngay_nhan', '=', $nam)->max('so_den');
            } else {
                $soDenvb = VanBanDen::where([
                    'van_ban_chi_nhanh' => null,
                    'don_vi_id' => auth::user()->donVi->parent_id,
                    'type' => 2
                ])->whereYear('ngay_nhan', '=', $nam)->max('so_den');
            }
...
//            }
        ...
        $ds_loaiVanBan = LoaiVanBan::wherenull('deleted_at')->whereIn('loai_van_ban', [1, 3])
...
//                    $vanbandv->chuc_vu = $chucvu;
    ...
    public function kiemTraTrichYeu(Request $request)
    {
        ...
        if ($user->hasRole(VAN_THU_HUYEN)) {
            $data = VanBanDen::where(['so_ky_hieu' => $so_ky_hieu, 'type' => 1])
                ...
                ->whereNull('deleted_at')
                ->take(5)->get();
            $data2 = VanBanDen2021::where(['so_ky_hieu' => $so_ky_hieu, 'type' => 1])
                ->orderBy('id', 'desc')
                ->whereNull('deleted_at')
                ->take(5)->get();
            ...
            $data = VanBanDen::where(['so_ky_hieu' => $so_ky_hieu, 'type' => 2, 'don_vi_id' => auth::user()->donVi->parent_id])
                ...
                ->orderBy('id', 'desc')
                ->take(5)->get();
            $data2 = VanBanDen2021::where(['so_ky_hieu' => $so_ky_hieu, 'type' => 2, 'don_vi_id' => auth::user()->donVi->parent_id])
                ->whereNull('deleted_at')
                ->orderBy('id', 'desc')
                ->take(5)->get();
        }
        ...
        $returnHTML2 = $data2->isNotEmpty() ? view('vanbanden::van_ban_den.check_trung_van_ban1',
            compact('ds_nguoiDung', 'ds_nguoiKy', 'data2', 'ds_loaiVanBan'))->render() : '';
        return response()->json(
            [
                'is_relate' => $data->isNotEmpty() ? true : false,
                'is_relate2' => $data2->isNotEmpty() ? true : false,
                'html' => $returnHTML,
                'html2' => $returnHTML2,
            ]
        );
    ...
    public function capNhatVanBanTonPhoPhong()
    {
        ...
        $userIDPhoPhong = User::where('don_vi_id', auth::user()->don_vi_id)->Role(PHO_PHONG)->get();
        ...
        foreach ($userIDPhoPhong as $da) {
            ...
            foreach ($danhSachVanBanDenQuaHanDangXuLy as $data) {
                ...
                $vaBanDenID->trinh_tu_nhan_van_ban = 7;
                $vaBanDenID->save();
            }
    ...
    public function capNhatVanBanTonTruongPhong()
    {
        ...
        $userIDPhoPhong = User::where('don_vi_id', auth::user()->don_vi_id)->Role(TRUONG_PHONG)->get();
...
//                })
            ...
            $danhSachVanBanDenkhan = VanBanDen::where(function ($query) use ($da) {
                if (!empty($da)) {
                    return $query->whereHas('vanBanDangXuLy', function ($q) use ($da) {
                        return $q->where('can_bo_nhan_id', $da->id);
                    });
...
//                })
                ->select('id')
                ->get();
            dd($danhSachVanBanDenTrongHanDangXuLy->pluck('id'), $danhSachVanBanDenQuaHanDangXuLy->pluck('id'), $danhSachVanBanDenkhan->pluck('id'));
    ...
    public function capNhatVanBanTonChuyenVien()
    {
        ...
        $userIDPhoPhong = User::where('don_vi_id', auth::user()->don_vi_id)->Role(CHUYEN_VIEN)->get();
        ...
        foreach ($userIDPhoPhong as $da) {
            ...
            $danhSachVanBanDenQuaHanDangXuLy = VanBanDen::where(function ($query) use ($da) {
                ...
                ->select('id')
                ->get();
            dd($danhSachVanBanDenQuaHanDangXuLy);
            foreach ($danhSachVanBanDenQuaHanDangXuLy as $data) {
                $vaBanDenID = VanBanDen::where('id', $data->id)->first();
                $vaBanDenID->trinh_tu_nhan_van_ban = 7;
                $vaBanDenID->save();
