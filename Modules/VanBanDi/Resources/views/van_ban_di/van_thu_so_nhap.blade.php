                    ...
                    <div class="box-body">
                        <form class="form-row"
                            ...
                            <div class="form-group col-md-3">
                                ...
                                <select class="form-control show-tick so-van-ban" autofocus name="sovanban_id" required>
                            ...
                            <div class="form-group col-md-3">
                                ...
                                <select class="form-control show-tick loai-van-ban"  name="loaivanban_id" id="loaivanban_id" required>
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    <label for="exampleInputEmail3">Số đi văn bản <span class="color-red">*</span></label>
                                    <input type="text" class="form-control " value="" readonly name="so_di" id="exampleInputEmail3"
                                           placeholder="Số đi" style="font-weight: 800;color: #F44336;cursor: not-allowed;" >
                                </div>
                            </div>
                                    ...
                                    <div class=" col-md-3 form-group">
                                        ...
                                        <select class="form-control select2" name="phong_phat_hanh" id=""  required>
                                            ...
                                            @foreach ($ds_DonVi_phatHanh as $DonVi_phatHanh)
                                                <option value="{{ $DonVi_phatHanh->id }}" {{auth::user()->hasRole(VAN_THU_HUYEN) && $donViSo->don_vi_id == $DonVi_phatHanh->id    ? 'selected ': ''}} >{{ $DonVi_phatHanh->ten_don_vi }}</option>
                                            @endforeach
                                        </select>
                                    </div>

                                    <div class="col-md-3  mt-1 ">
                                        <label for="sokyhieu" class="col-form-label">File văn bản đi</label>
                                        <div class="form-line input-group control-group">
                                            <input type="file" id="url-file" multiple name="file[]" class="form-control">
                                        </div>
                                    </div>
                                </div>
                            </div>
...
@section('script')
    ...
    <script type="text/javascript">
        ...
        $('.loai-van-ban').on('change', function (e) {
            var loai_van_ban = $('[name=loaivanban_id]').val();
            var so_van_ban = $('[name=sovanban_id]').val();
            e.preventDefault();
            $.ajax({
                ...
                // hideLoading();
                $("input[name='vb_sokyhieu']").val(res.html);
                $("input[name='so_di']").val(res.soDi);


            });
        });
        $('.so-van-ban').on('change', function (e) {
            var loai_van_ban = $('[name=loaivanban_id]').val();
            var so_van_ban = $('[name=sovanban_id]').val();
            e.preventDefault();
            $.ajax({
